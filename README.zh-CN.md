...
**v0.7.0** · DeepSeek Harness `0.1.1-rc.2` / `0.1.5-rc.2` · GitHub `dsh-coding-remote-kit`
...
> **升级：** 按 [`INSTALL.md`](INSTALL.md) 的版本化步骤操作。`0.7.0` 通过宿主 `sessionController` 服务适配 DSH `0.1.5-rc.2`——会话列表、历史分页、发送、取消、实时事件、手机审批与提问、助手流式回复——同时 `0.1.1-rc.2` 继续走旧 `apiProxy` 路径；`0.6.0` 增加手机 Queue/Steer、活动条、历史游标分页与可选离线推送；`0.5.2` 增加 CSP/配对加固与 sessionStorage 密钥；`0.5.1` 修复 Settings 安装 cloudflared 后无需重载即可 Start；`0.5.0` 增加连接诊断、Quick Tunnel 免责勾选与 cloudflared 钉死校验；保留 profile、存储与配对文件，所有选定插件更新后再重启一次现有 DSH Web 进程。`dsh-coding-oauth-core@0.1.0` 仍是 Hub/Subscription 的共享 npm 依赖，不是需要单独安装的 DSH 插件。
...
| | 请用这个 | 说明 |
|---|---|---|
| npm | `dsh-coding-remote-kit@0.7.0` | `dsh plugin --profile web add dsh-coding-remote-kit@0.7.0` |
