...
## Candidato 0.6.0: recuperação e aprovações

Este checkout gera um **tarball candidato 0.6.0**, não uma publicação npm. Use as instruções de tarball em [INSTALL.md](INSTALL.md); os exemplos npm mantêm a versão de referência. A revogação fecha as conexões LAN, túnel e relay daquele dispositivo; a atividade autenticada atualiza o prazo de inatividade existente. Rascunhos, sessão atual e posição de leitura ficam no sessionStorage, separados por host/dispositivo/sessão. A recuperação da rede ou o retorno ao primeiro plano permite nova autenticação; um envio incerto é preservado para conferência, nunca reenviado automaticamente. Uma nova aba ainda exige pareamento. As notificações são opcionais e desativadas por padrão: observam pendências mesmo sem telefone conectado, preservam o RPC ID original e evitam repetir entregas bem-sucedidas após replay do mux. Um alvo ausente é explicado sem afirmar que foi resolvido; um replay tardio ainda pode localizar o cartão.
